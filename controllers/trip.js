...
// * Propose
router.get('/:tripId/propose', isSignedIn, async (req, res, next) => {
  try {
    const { tripId } = req.params
    const trip = await Trip.findById(tripId)
    if (!trip) throw new NotFound()
    ...
    const prompt = [
      ...
      },
      {
        ...
        content: `If the user provides data about a trip that you are asked to extend, use the existing data to guide your proposals.
          ...
          Reuse the user's data where present. Don't change or correct it`,
      },
      {
        role: 'user',
        content: 'I am planning a trip. Here is my current plan in JSON format.',
      },
      {
        role: 'assistant',
      ...
      },
      {
        ...
        content: `Expand the plan.
            ...
            Suggest 3 activities at max for each travel day, and 21 at max for the full trip.
    ...
    const responseToClient = {
      ...
      trip: openAIresponse.output_parsed,
    }
